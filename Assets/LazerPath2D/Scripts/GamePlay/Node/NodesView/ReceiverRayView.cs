...
{
    public class ReceiverRayView : NodeView
    {
        private const string StepOpacity = "_StepOpacity";
        ...
        private float _colorIntensity = 6.5f;

        private Coroutine _coroutineEffect;
        private Coroutine _coroutineDissolve;
        ...
        private void OnDisable()
        {
            ...
            if (_coroutineDissolve != null)
                this.StopCoroutine(_coroutineDissolve);
        }
        ...
        public void ToStartDissolve()
        {
            Material material = BackgroundSprite.material;

            _coroutineDissolve = this.StartCoroutine(StartDissolve(material));
        }
        ...
        private IEnumerator StartDissolve(Material material)
        {
            float stepOpacityValue = 0;

            while (stepOpacityValue < 1)
            {
                stepOpacityValue = Mathf.Clamp01(stepOpacityValue + Time.deltaTime);

                material.SetFloat(StepOpacity, stepOpacityValue);
